...
# Defining Tabs - NEW TAB ADDED HERE
tab1, tab2, tab3, tab4 = st.tabs([
    "🤖 Real-Time AI Signals",
    "💼 Live Paper Trading",  # NEW SECTION
    "📊 HD Daily RRG Chart",
    "📸 Chart Analyzer",
])
...
with tab2:
    ...
    # Fetch live price for the selected pair instantly
    ...
    if st.button(f"Place {trade_type} Order"):
        live_entry_price = get_current_price(trade_pair) # Fetch again on click to be precise
        if live_entry_price:
            # Check balance
            if amount_usd > st.session_state.balance:
                ...
                st.success(f"{trade_type} order for ${amount_usd} of {trade_pair} placed successfully at ${live_entry_price:,.2f}")
                time.sleep(1)
                st.rerun()
            ...
            for i, pos in enumerate(st.session_state.positions):
                if pos['id'] == pos_to_close_id:
                    ...
                    if close_price:
                        ...
                        st.success(f"Position closed at ${close_price:,.2f} with P&L: ${final_pnl:,.2f}. Balance updated.")
                        time.sleep(1)
                        st.rerun()
                    else:
                        st.error("Error fetching live price to close. Try again.")
                    break
