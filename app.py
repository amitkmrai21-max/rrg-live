            ...
            if df is not None and not df.empty:
                ...
                if response_text:
                    ...
                    if sma20 > sma50 and rsi < 70:
                        ...
                        tp = f"${price * 1.015:.2f}"
                        sl = f"${price * 0.990:.2f}"
                        reason = "SMA Crossover is Bullish & RSI is under control."
...
# --- TAB 3: CHART ANALYZER (MULTI-MODEL FALLBACK) ---
with tab3:
    ...
    if uploaded_file and client:
        ...
        if st.button("Analyze Image with AI"):
            with st.spinner("Analyzing chart image with AI..."):
                resp_text = None
                img_prompt = "Provide Buy/Sell decision, Entry Price Range, Target Price (TP), and Stop-Loss (SL) for this chart in clear bullet points."

                # Try 3 different models sequentially if one is rate limited
                models_to_try = [
                    "gemini-2.0-flash",
                    "gemini-1.5-flash",
                    "gemini-1.5-flash-8b",
                ...
                if resp_text:
                    st.success("🎯 Chart Analysis Output:")
                    st.markdown(resp_text)
                else:
                    st.error(
                        "Rate limit reached on all free models. Please wait 30 seconds and try clicking again."
                    )
