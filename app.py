...
try:
    ...
    st.error("⚠️ GEMINI_API_KEY Streamlit Secrets me set nahi hai! Settings -> Secrets me set karein.")

st.title("🚀 AI Real-Time Trading Assistant & RRG")

tab1, tab2, tab3 = st.tabs(["🤖 Real-Time AI Signals", "📊 RRG Rotation Chart", "📸 Chart Analyzer"])
...
with tab1:
    ...
    pair = st.selectbox("Crypto Pair Select Karein:", ["BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT"])
    timeframe = st.selectbox("Timeframe Select Karein:", ["15m (Scalping)", "1h (Intraday)"])
    ...
    if st.button("🤖 AI Signal Decision Fetch Karein"):
        with st.spinner("Real-time market data & AI Analysis chal raha hai..."):
            ...
            if df is not None and not df.empty:
                ...
                st.write("### Live Market Indicators:")
                ...
                if client:
                    ...
                    st.error("Gemini Client ready nahi hai. API Key check karein.")
            else:
                st.error("Market data fetch nahi ho pa raha hai.")

# TAB 2: REAL-TIME RRG CHART
with tab2:
    st.subheader("🌐 Relative Rotation Graph (RRG) - Market Momentum")
    st.info("RRG quadrant chart batata hai kaunsa coin Leading (Strong) hai aur kaunsa Lagging (Weak) BTC benchmark ke mukable.")
    
    if st.button("Generate Live RRG Chart"):
        with st.spinner("Relative Rotation plot ho raha hai..."):
            symbols = ["ETH-USD", "SOL-USD", "XRP-USD", "ADA-USD"]
            benchmark = "BTC-USD"
            
            try:
                bm_data = yf.download(benchmark, period="1mo", interval="1d", progress=False)
                ...
                ax.text(102, 102, "LEADING (Strong Buy)", color='green', fontsize=10, weight='bold')
                ax.text(96, 102, "WEAKENING (Take Profit)", color='orange', fontsize=10, weight='bold')
                ax.text(96, 98, "LAGGING (Avoid/Sell)", color='red', fontsize=10, weight='bold')
                ax.text(102, 98, "IMPROVING (Watch/Buy)", color='blue', fontsize=10, weight='bold')
                
                for sym in symbols:
                    s_data = yf.download(sym, period="1mo", interval="1d", progress=False)
                    ...
                    ax.annotate(sym.replace("-USD", ""), (x + 0.2, y + 0.2), fontsize=11, weight='bold')
                ...
                st.error(f"RRG Calculation Error: {e}")
            ...
            with st.spinner("Analyzing image..."):
                try:
                    resp = client.models.generate_content(
                        model="gemini-2.5-flash",
                        contents=[img, "Provide Buy/Sell signal, Target and Stoploss for this chart."]
                    )
                    st.write(resp.text)
                except Exception as e:
