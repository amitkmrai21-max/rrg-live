            ...
            try:
                ...
                if bm_close.empty or len(bm_close) < 20:
                    ...
                    if not plotted_any:
                        ...
                        # Save as PNG bytes and persist in session_state so the
                        # chart survives reruns triggered by Paper Trading's
                        # auto-refresh (won't "close" on its own anymore)
                        buf = io.BytesIO()
                        fig.savefig(buf, format="png", facecolor=fig.get_facecolor())
                        st.session_state.rrg_chart_bytes = buf.getvalue()
                        st.session_state.rrg_generated_at = datetime.now().strftime("%H:%M:%S")
            except Exception as e:
                st.error(f"Error: {e}")

    if st.session_state.get("rrg_chart_bytes"):
        st.caption(f"Last generated: {st.session_state.get('rrg_generated_at', '')}")
        st.image(st.session_state.rrg_chart_bytes, use_container_width=True)
