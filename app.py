        ...
        with st.spinner("Market Data & AI Analysis chal raha hai..."):
            ...
            if df is not None and not df.empty:
                ...
                if response_text:
                    ...
                    st.error("⚠️ AI Signal fetch nahi ho saka. Key ya quota check karein.")
            else:
...
with tab2:
    ...
    with c2:
        ...
        st.markdown(
            f"#### Live Portfolio P&L: <span style='color:{pnl_color};'>${current_pnl:,.2f}</span>",
    ...
    if not st.session_state.positions:
        ...
        df_display = pd.DataFrame([{
            ...
            "Pair": p["symbol"],
            "Type": p["type"],
            "Amt (USD)": f"${p['amount_usd']:,.2f}",
            "Entry Price": f"${p['entry_price']:,.2f}",
            "Live P&L": f"${p['live_pnl']:,.2f}",
        ...
        with col_c1:
            pos_id = st.selectbox(
                "Select Position ID to close:", df_display["ID"].tolist()
            )
