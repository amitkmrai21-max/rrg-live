...
tab1, tab2, tab3 = st.tabs(["🤖 Real-Time AI Signals", "📊 Daily RRG Chart (With BTC)", "📸 Chart Analyzer"])
            ...
            if df is not None and not df.empty:
                ...
                if client:
                    try:
                        ...
                        """
                        # Updated Gemini Model Name to fix 404 Error
...
# --- TAB 2: DAILY RRG CHART WITH BTC & ARROWS ---
with tab2:
    st.subheader("📊 Daily Relative Rotation Graph (BTC Included)")
    st.write("Ab BTC samet sabhi major coins ka rotation visual movement (Arrow) dikhega.")
    
    if st.button("Generate Daily RRG Chart"):
        with st.spinner("Daily RRG Arrow Plot ho raha hai..."):
            # BTC is now included in symbols, using USD cash as baseline
            symbols = ["BTC-USD", "ETH-USD", "SOL-USD", "XRP-USD", "ADA-USD"]
            benchmark = "USDT-USD" # Benchmark changed to USDT to show BTC trajectory
            ...
            try:
                ...
                ax.text(101.5, 101.5, "LEADING 🟢\n(Strong Buy)", color='green', fontsize=10, weight='bold')
                ax.text(97, 101.5, "WEAKENING 🟠\n(Take Profit)", color='orange', fontsize=10, weight='bold')
                ax.text(97, 98.5, "LAGGING 🔴\n(Sell / Avoid)", color='red', fontsize=10, weight='bold')
                ax.text(101.5, 98.5, "IMPROVING 🔵\n(Watch / Buy)", color='blue', fontsize=10, weight='bold')
                ...
                for sym in symbols:
                    ...
                    # Last 4 Days Trail for Arrow Plotting
                ...
                ax.set_xlabel("JDK RS-Ratio (Daily Trend Strength)")
                ax.set_ylabel("JDK RS-Momentum (Daily Speed)")
                ax.set_title("Crypto Daily RRG Rotation Trajectory")
                ax.grid(True, alpha=0.3)
                
                st.pyplot(fig)
            except Exception as e:
                st.error(f"RRG Arrow Error: {e}")
