...
def extract_rrg_series(data, ticker):
    try:
        return data[ticker]["Close"].dropna()
    except Exception:
        return pd.Series(dtype=float)
...
def build_rrg_figure(data, symbols, benchmark):
    benchmark_close = extract_rrg_series(data, benchmark)
    ...
    for idx, ticker in enumerate(symbols):
        coin_close = extract_rrg_series(data, ticker)
        if coin_close.empty:
            continue
