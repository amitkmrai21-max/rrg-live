...
tab1, tab2, tab3 = st.tabs(["🤖 Real-Time AI Signals", "📊 HD Daily RRG Chart (Clear BTC)", "📸 Chart Analyzer"])
        ...
        with st.spinner("Market Data & AI Analysis chal raha hai..."):
            ...
            if df is not None and not df.empty:
                ...
                if client:
                    ...
                    # Quota Error Bypass Logic (Tries Multiple Models)
                    response_text = None
                    ...
                    st.markdown("---")
                    st.subheader("🎯 AI Signal Output:")
                    if response_text:
                        st.success(response_text)
                    else:
                        # Backup Technical Output if API Quota Limit hits
                        action = "BUY 🟢" if (sma20 > sma50 and rsi < 70) else ("SELL 🔴" if rsi > 70 else "WAIT ⚪")
                        st.warning("⚠️ Free API Quota Rate Limit reached. Rule-Based Fallback Signal:")
                        st.markdown(f"### ACTION: {action}")
                        st.markdown(f"- **Current Price**: ${price:.2f}\n- **RSI**: {rsi:.1f}\n- **Trend**: {'Upward' if sma20>sma50 else 'Downward'}")
                else:
...
with tab2:
    ...
    st.write("BTC ki trail line ab lambi (7-day movement) aur highlighted box me spash dikhegi.")
    ...
    if st.button("Generate Large HD RRG Chart"):
        with st.spinner("HD Chart plot ho raha hai..."):
            ...
            try:
                ...
                # Extra Large Size (14x10) for Maximum Clarity
                ...
                ax.text(101.5, 101.5, "LEADING 🟢\n(Strong Buy Zone)", color='green', fontsize=13, weight='bold')
                ax.text(97, 101.5, "WEAKENING 🟠\n(Take Profit Zone)", color='darkorange', fontsize=13, weight='bold')
                ax.text(97, 98.5, "LAGGING 🔴\n(Sell / Avoid Zone)", color='red', fontsize=13, weight='bold')
                ax.text(101.5, 98.5, "IMPROVING 🔵\n(Watch / Recovery Zone)", color='blue', fontsize=13, weight='bold')
                ...
                for sym in symbols:
                    ...
                    # 7-Days Trail for BTC and other coins for longer/clearer arrows
                    trail_len = 7
                    ...
                    # Trail Plot
                    ax.plot(x_vals, y_vals, linestyle='-', alpha=0.8, linewidth=line_width, color=line_color)
                    ...
                    ax.annotate(
                        ...
                        arrowprops=dict(arrowstyle="-|>", lw=line_width+1, color='red' if is_btc else 'black', mutation_scale=18)
                    )
                    
                    # Clear Labeling (BTC gets special Red Highlighted Box)
                    ...
                    ax.annotate(
                        ...
                        weight='bold',
                        color=txt_color,
                        bbox=dict(boxstyle="round,pad=0.4", fc=box_color, ec="black", lw=1.5, alpha=0.9)
                    )
                    
                ax.set_xlabel("JDK RS-Ratio (Daily Trend Strength)", fontsize=13, weight='bold')
                ax.set_ylabel("JDK RS-Momentum (Daily Speed)", fontsize=13, weight='bold')
                ax.set_title("Crypto Daily RRG Rotation Trajectory vs USDT", fontsize=16, weight='bold')
