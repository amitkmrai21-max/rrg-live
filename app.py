...
with tab3:
    ...
    # Total time to glide from one day to the next (split into smaller sub-steps below)
    day_duration_ms = 2200 if speed_label == "Slow" else 3800

    # Auto-refresh every 60s, but only once the chart has been generated at
    # least once — avoids surprising an empty tab into constantly rerunning.
    _rrg_autorefresh_count = st_autorefresh(interval=60000, key="rrg_autorefresh")
    _prev_count = st.session_state.get("rrg_autorefresh_prev", 0)
    auto_triggered = _rrg_autorefresh_count > _prev_count and st.session_state.get("rrg_fig") is not None
    st.session_state.rrg_autorefresh_prev = _rrg_autorefresh_count

    if st.session_state.get("rrg_fig") is not None:
        st.caption("🔄 Auto-refreshing every 60s")

    manual_clicked = st.button("Generate Large HD RRG Chart", key="gen_rrg_btn")

    if manual_clicked or auto_triggered:
        with st.spinner("Plotting chart..."):
            ...
            try:
                ...
                # Also fetch TODAY's live (intraday) price for every ticker so
                # the chart isn't stuck 1-2 days behind Yahoo's delayed daily close
                live_ts = pd.Timestamp.now()
                try:
                ...
                def get_live_close(t):
                    if live_data is None:
                        return None
                    try:
                        s = live_data[t]["Close"].dropna()
                        return float(s.iloc[-1]) if not s.empty else None
                    except Exception:
                        return None

                def get_close_with_live(t):
                    s = get_close(t)
                    live_price = get_live_close(t)
                    if live_price is not None and not s.empty and live_ts > s.index[-1]:
                        s = pd.concat([s, pd.Series([live_price], index=[live_ts])])
                    return s

                bm_close = get_close_with_live(benchmark)
                ...
                if bm_close.empty or len(bm_close) < 20:
                    ...
                    series_map = {}
                    for sym in symbols:
                        s_close = get_close_with_live(sym)
                        if s_close.empty:
                            continue
                        ...
                        if len(frame_dates) < 2:
                            ...
                            def build_frame_traces(up_to_date):
                                ...
                                for name, df_ in series_map.items():
                                    ...
                                    # Highlighted marker for TODAY's point only (kept small so it
                                    # doesn't dominate the chart)
                                    traces.append(go.Scatter(
                                        x=[sub["x"].iloc[-1]], y=[sub["y"].iloc[-1]],
                                        mode="markers",
                                        marker=dict(size=13, color=color_map[name], line=dict(color="white", width=2)),
                                        name=name,
                                        legendgroup=name,
                                        showlegend=False,
                            ...
                            def build_interp_traces(d_prev, d_next, t):
                                # Smoothly glides only the newest point from d_prev's
                                # position to d_next's position — the rest of the trail
                                # stays put — so movement looks gradual, not a jump.
                                traces = []
                                for name, df_ in series_map.items():
                                    if d_prev not in df_.index or d_next not in df_.index:
                                        continue
                                    trail_fixed = df_.loc[:d_prev].tail(TRAIL - 1)
                                    x_prev, y_prev = df_.loc[d_prev, "x"], df_.loc[d_prev, "y"]
                                    x_next, y_next = df_.loc[d_next, "x"], df_.loc[d_next, "y"]
                                    xi = x_prev + (x_next - x_prev) * t
                                    yi = y_prev + (y_next - y_prev) * t
                                    xs = list(trail_fixed["x"]) + [xi]
                                    ys = list(trail_fixed["y"]) + [yi]
                                    traces.append(go.Scatter(
                            ...
                            # Build the animation as many small glide-steps instead of
                            # one big jump per day — same 20-day time window overall,
                            # but each visual step covers a much smaller distance.
                            SUBSTEPS = 8
                            frames = []
                            slider_steps = []
                            for i, d in enumerate(frame_dates):
                                fname = f"d{i}"
                                frames.append(go.Frame(data=build_frame_traces(d), name=fname))
                                slider_steps.append((fname, d.strftime("%d/%m")))
                                if i < len(frame_dates) - 1:
                                    d_next = frame_dates[i + 1]
                                    for s in range(1, SUBSTEPS):
                                        t = s / SUBSTEPS
                                        frames.append(go.Frame(
                                            data=build_interp_traces(d, d_next, t),
                                            name=f"d{i}_s{s}",
                                        ))

                            frame_ms = max(int(day_duration_ms / SUBSTEPS), 60)
                            ...
                            fig.update_layout(
                                    ...
                                    buttons=[
                                        dict(
                                            ...
                                            args=[None, {
                                                ...
                                                "fromcurrent": True,
                                                "transition": {"duration": int(frame_ms * 0.9), "easing": "linear"},
                                            }],
                                        ),
                                        dict(
                                            label="⏸ Pause",
                                ...
                                sliders=[dict(
                                    ...
                                    steps=[
                                        ...
                                        for fname, label in slider_steps
                                    ],
                                )],
                            )
