...
# --- TAB 2: LIVE PAPER TRADING (WITH AUTO REFRESH) ---
with tab2:
    st.subheader("💼 Live Paper Trading Simulator")

    # AUTO RUN EVERY 5000ms (5 Seconds) IF POSITIONS EXIST
    if len(st.session_state.positions) > 0:
        autorun(interval=5000, key="trade_auto_refresh")
    ...
    with c2:
        # Profit = Blue (#1E90FF), Loss = Red (#FF4500)
    ...
    if st.button(f"Place {trade_type} Order", key="place_order_btn"):
        if not live_entry_price:
            ...
            save_trade_data()  # Save permanently
