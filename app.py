        ...
        with st.spinner("Market Data & AI Analysis chal raha hai..."):
            ...
            if df is not None and not df.empty:
                ...
                if response_text:
                    ...
                    # ALWAYS SHOW EXACT ENTRY, TARGET, STOPLOSS EVEN ON FALLBACK
                    if sma20 > sma50 and rsi < 70:
                        action = "BUY 🟢"
                        entry = f"${price:.2f}"
                        tp = f"${price * 1.015:.2f}"  # 1.5% Target
                        sl = f"${price * 0.990:.2f}"  # 1.0% Stoploss
                        reason = "SMA Crossover is Bullish & RSI is under control."
                    elif rsi > 70 or sma20 < sma50:
                        action = "SELL 🔴"
                        entry = f"${price:.2f}"
                        tp = f"${price * 0.985:.2f}"  # 1.5% Short Target
                        sl = f"${price * 1.010:.2f}"  # 1.0% Stoploss
            ...
            with st.spinner("Analyzing image..."):
                try:
                    resp = client.models.generate_content(
                        ...
                        contents=[
                            img,
                            "Provide Buy/Sell decision, Entry, Target and Stoploss for this chart.",
                        ],
                    )
                    st.write(resp.text)
                except Exception as e:
