...
def get_crypto_data(symbol: str, period="3mo", interval="1d"):
...
def analyze_volume(df):
    ...
    latest_ratio = vol_ratio.iloc[-1] if len(vol_ratio) > 0 and not np.isnan(vol_ratio.iloc[-1]) else 1.0
    spike = "NONE"
    if latest_ratio > 2.5:
        spike = "STRONG_SPIKE"
...
def calculate_volume_profile(df, num_candles=30):
    if len(df) < num_candles or 'Volume' not in df.columns or df['Volume'].sum() == 0:
...
def detect_candlestick_patterns(df):
    ...
    if HAS_TALIB:
        try:
            patterns['Hammer'] = int(talib.CDLHAMMER(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
            patterns['Engulfing'] = int(talib.CDLENGULFING(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
            patterns['Harami'] = int(talib.CDLHARAMI(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
            patterns['Morning_Star'] = int(talib.CDLMORNINGSTAR(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
            patterns['Evening_Star'] = int(talib.CDLEVENINGSTAR(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
            patterns['Doji'] = int(talib.CDLDOJI(df['Open'], df['High'], df['Low'], df['Close']).iloc[-1])
        except Exception:
            pass
...
def calculate_signal_score(trend, bos, choch, sweeps, fvg, ob, divergence, vol_spike, vol_ratio, candle_signal, confluence_score, rsi, sma_signal):
    ...
    final_score = max(0, min(100, int(50 + score)))
    ...
    if action == "BUY":
        ...
        for pos in st.session_state.positions:
            if pos["symbol"] == symbol:
                ...
                break
        if not pos_to_close:
            return False, "No open position found to sell."
...
# ──────────────────────────────────────────────────────────────
# Relative Rotation Graph (RRG) Calculation & Plot
# ──────────────────────────────────────────────────────────────
@st.cache_data(ttl=300, show_spinner=False)
def calculate_rrg_data():
    all_tickers = RRG_SYMBOLS + [RRG_BENCHMARK]
    df = yf.download(all_tickers, period="90d", interval="1d", auto_adjust=False, progress=False)
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        closes = df
    if RRG_BENCHMARK not in closes.columns:
        return None
    
    rrg_results = {}
    bench = closes[RRG_BENCHMARK]
    for sym in RRG_SYMBOLS:
        if sym in closes.columns:
            rs = 100 * (closes[sym] / bench)
            rs_ratio = 100 + ((rs - rs.rolling(14).mean()) / rs.rolling(14).std(ddof=0))
            rs_momentum = 100 + ((rs_ratio - rs_ratio.rolling(10).mean()) / rs_ratio.rolling(10).std(ddof=0))
            clean_df = pd.DataFrame({"RS_Ratio": rs_ratio, "RS_Momentum": rs_momentum}).dropna()
            if len(clean_df) >= 5:
                rrg_results[sym] = clean_df.tail(5)
    return rrg_results
...
with tab1:
    ...
    if data is not None and not data.empty:
        ...
        fig.update_layout(title=f"{selected} Price & Indicators", height=500, xaxis_rangeslider_visible=False, template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Current Price", f"${price:.6f}")
        col_m2.metric("RSI (14)", f"{data['RSI'].iloc[-1]:.2f}")
        col_m3.metric("SMA20 / SMA50", f"${data['SMA20'].iloc[-1]:.2f} / ${data['SMA50'].iloc[-1]:.2f}")
    else:
        st.warning("No data available.")

with tab2:
    st.header("Daily Relative Rotation Graph (RRG vs BTC)")
    rrg_dict = calculate_rrg_data()
    if rrg_dict:
        fig_rrg = go.Figure()
        fig_rrg.add_shape(type="rect", x0=100, y0=100, x1=120, y1=120, fillcolor="rgba(0, 255, 0, 0.1)", line_width=0)
        fig_rrg.add_shape(type="rect", x0=100, y0=80, x1=120, y1=100, fillcolor="rgba(255, 255, 0, 0.1)", line_width=0)
        fig_rrg.add_shape(type="rect", x0=80, y0=80, x1=100, y1=100, fillcolor="rgba(255, 0, 0, 0.1)", line_width=0)
        fig_rrg.add_shape(type="rect", x0=80, y0=100, x1=100, y1=120, fillcolor="rgba(0, 0, 255, 0.1)", line_width=0)
        
        for sym, df_tail in rrg_dict.items():
            fig_rrg.add_trace(go.Scatter(
                x=df_tail["RS_Ratio"],
                y=df_tail["RS_Momentum"],
                mode="lines+markers+text",
                name=sym,
                text=["" for _ in range(len(df_tail)-1)] + [sym],
                textposition="top center"
            ))
        fig_rrg.add_hline(y=100, line_dash="dash", line_color="white")
        fig_rrg.add_vline(x=100, line_dash="dash", line_color="white")
        fig_rrg.update_layout(
            title="Crypto Rotation vs BTC Benchmark (Leading, Weakening, Lagging, Improving)",
        ...
        st.warning("Failed to fetch or compute RRG data. Please check network/Yahoo Finance rate limits.")

with tab3:
    st.header("Live Paper Trading")
    col1, col2 = st.columns([2, 1])
    with col1:
        st.metric("Wallet Balance", money(st.session_state.balance))
        if st.session_state.positions:
            st.subheader("Open Positions")
            for pos in st.session_state.positions:
                current = get_current_price(pos["symbol"]) or pos["entry_price"]
                pnl = (current - pos["entry_price"]) * pos["quantity"]
                st.write(f"🔹 **{pos['symbol']}**: {pos['quantity']} @ ${pos['entry_price']:.4f} | Current: ${current:.4f} | P&L: ₹{pnl:,.2f}")
        else:
            st.info("No open positions.")
            
        if st.session_state.trade_history:
            with st.expander("📜 Trade History"):
                st.json(st.session_state.trade_history[-10:])
    with col2:
